...
- Расшифровывает через [Kesha Voice Kit](https://github.com/drakulavich/kesha-voice-kit) — Rust-движок на CoreML/Apple Neural Engine, заметно быстрее обычного Whisper на Apple Silicon
- Вставляет текст через буфер обмена (⌘V) в любое приложение
- Работает в фоне, запускается автоматически при входе

**Требования:** macOS 13+ (на Apple Silicon работает на Neural Engine, на Intel — через ONNX, медленнее), Python 3.11–3.13 (рекомендуется установить с [python.org](https://python.org/downloads/)), Bun (ставится автоматически), ~3 ГБ свободного места (движок + модели Kesha).
...
Скрипт сам:
- найдёт Python
- установит зависимости (Python-пакеты + Bun + Kesha Voice Kit)
- скомпилирует вспомогательный .app
- создаст launchd-агент (автозапуск)
- скачает движок и модели Kesha (~2.5 ГБ, один раз)
...
|---|---|---|
| `language` | `"ru"` | Язык. `"en"`, `"uk"`, `"de"` и др. — см. [список языков Kesha](https://github.com/drakulavich/kesha-voice-kit/blob/main/docs/languages.md). |
...
│
├── В логе: "Тишина" (Kesha ничего не распознал)
...
|---|---|---|
| `kesha не найден` | Kesha не установлена | `bun add -g @drakulavich/kesha-voice-kit && kesha install` |
| `No module named 'sounddevice'` (или другой пакет) | Python-зависимости не установлены | `pip3 install sounddevice numpy pyobjc-framework-Quartz pyobjc-framework-Cocoa pyobjc-framework-UserNotifications pyobjc-framework-AVFoundation soundfile` |
...
sounddevice записывает микрофон в RAM
        ↓ (второе нажатие)
Kesha (CoreML/Apple Neural Engine) расшифровывает аудио
        ↓
...
| Пакет | Зачем |
|---|---|
| [Kesha Voice Kit](https://github.com/drakulavich/kesha-voice-kit) (через Bun) | расшифровка речи (CoreML/Apple Neural Engine) |
...
# Удалить Kesha (опционально, если не нужна больше)
